...
export class MomentsController {
    ...
    @ApiNoContentResponse({
        description: "No moments available. No response body."
    })
    @ApiNotFoundResponse({
    ...
    async getMomentFeed(@Req() req, @Res({ passthrough: true }) res: Response) {
    ...
    async createMoment(@Req() req, @Body() dto: CreateMomentDto, @UploadedFiles() photoFiles: any[] = []) {
        ...
        const rawIsPrivate =
            (req?.body?.isPrivate ?? req?.body?.privacy ?? dto.isPrivate ?? dto.privacy) as
                | boolean
                | string
                | null
                | undefined;
        const normalizedIsPrivate = this.normalizeBoolean(rawIsPrivate);
        const maybePrivacyPayload =
            normalizedIsPrivate === undefined ? {} : { isPrivate: normalizedIsPrivate };
    ...
    @ApiOperation({
        summary: "Confirm moment synced locally",
        description:
            "Call after the current device has downloaded and saved the moment locally. Server clears payload after both couple users confirm."
    })
    @ApiParam({
        name: "id",
        description: "Moment ID",
    ...
    @ApiNoContentResponse({
        description: "Moment sync confirmation saved. No response body."
    })
    @ApiNotFoundResponse({
        description: "Moment not found"
    })
    ...
    async confirmMomentSynced(@Req() req, @Param("id") id: string) {
        await this.momentsService.confirmMomentSynced(this.getCurrentUserId(req), id);
    }

    private normalizeBoolean(value?: boolean | string | null): boolean | undefined {
