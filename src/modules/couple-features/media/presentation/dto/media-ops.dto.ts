...
export class MediaItemResponseDto {
    ...
    @ApiPropertyOptional({
        description: "Server relay sync status",
        enum: ["uploaded", "partially_confirmed", "confirmed", "payload_deleted"],
        example: "uploaded",
        nullable: true
    })
    ...
    @ApiPropertyOptional({
        description: "User ids that confirmed this media item was saved locally",
        type: [String],
        example: ["7ad1fd3e-30ec-4cca-bfb9-9b8cb857ccf8"]
    })
    ...
    @ApiPropertyOptional({
        description: "Time when both users confirmed local sync",
    ...
    @ApiPropertyOptional({
        description: "Time when server cleared media payload",
        example: "2026-03-09T09:00:00.000Z",
        nullable: true
    })
