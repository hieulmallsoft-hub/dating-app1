...
export class MediaController {
    ...
    @ApiNoContentResponse({
        description: "No media available. No response body."
    })
    @ApiBadRequestResponse({
    ...
    @ApiOperation({
        summary: "Confirm media synced locally",
        description:
            "Call after the current device has downloaded and saved the media locally. Server clears payload after both couple users confirm."
    })
    @ApiParam({
        name: "id",
    ...
    @ApiNoContentResponse({
        description: "Media sync confirmation saved. No response body."
    })
    @ApiNotFoundResponse({
    ...
    async confirmMediaSynced(@Req() req, @Param("id") id: string) {
        await this.mediaService.confirmMediaSynced(this.getCurrentUserId(req), id);
    }
