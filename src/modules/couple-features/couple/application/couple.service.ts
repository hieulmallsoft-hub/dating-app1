...
import { Moment } from "../../moments/domain/entities/moment.entity";
import { Location } from "../../locations/domain/entities/location.entity";
...
export class CoupleService {
    ...
    private readonly disconnectedRetentionMs = 30 * 24 * 60 * 60 * 1000;
    ...
    async disconnect(userId: string) {
        const result = await this.dataSource.transaction(async (manager) => {
            ...
            couple.disconnectedAt = new Date();
    ...
    private async joinCoupleInternal(userId: string, inviteCode: string) {
        const result = await this.dataSource.transaction(async (manager) => {
            ...
            await this.purgeExpiredDisconnectedCouples(manager);
            ...
            const reusableCouple = await this.findReusableDisconnectedCouple(userId, partner.id, coupleRepo);
            if (reusableCouple) {
                reusableCouple.status = CoupleStatus.ACTIVE;
                reusableCouple.disconnectedAt = null;
                const savedCouple = await coupleRepo.save(reusableCouple);
            ...
            const staleCoupleIds = await this.findDisconnectedCoupleIdsForNewPair(manager, userId, partner.id);
            await this.purgeCoupleData(manager, staleCoupleIds);
    ...
    async cleanupExpiredDisconnectedCouples() {
        return this.dataSource.transaction((manager) => this.purgeExpiredDisconnectedCouples(manager));
    }
    ...
    private async purgeExpiredDisconnectedCouples(manager: EntityManager) {
        const cutoff = this.getDisconnectedRetentionCutoff();
        const coupleRepo = manager.getRepository(Couple);
        const expiredCouples = await coupleRepo.find({
            where: [
                {
        ...
        const coupleIds = expiredCouples.map((couple) => couple.id);

        return this.purgeCoupleData(manager, coupleIds);
    }

    private async findReusableDisconnectedCouple(
        userId: string,
        partnerId: string,
        coupleRepo: Repository<Couple>
    ) {
        const cutoff = this.getDisconnectedRetentionCutoff();
            ...
            where: [
                {
                    ...
                    user2Id: partnerId,
                    status: CoupleStatus.DISCONNECTED,
                    disconnectedAt: MoreThanOrEqual(cutoff)
                },
                {
                    user1Id: partnerId,
                    user2Id: userId,
    ...
    private async findDisconnectedCoupleIdsForNewPair(
        manager: EntityManager,
        userId: string,
        partnerId: string
    ) {
        const userIds = [userId, partnerId];
        const couples = await manager
            .getRepository(Couple)
            .createQueryBuilder("couple")
            .select("couple.id", "id")
            .where("couple.status = :status", { status: CoupleStatus.DISCONNECTED })
            .andWhere("(couple.user1Id IN (:...userIds) OR couple.user2Id IN (:...userIds))", { userIds })
            .andWhere(
                "NOT ((couple.user1Id = :userId AND couple.user2Id = :partnerId) OR (couple.user1Id = :partnerId AND couple.user2Id = :userId))",
                { userId, partnerId }
            )
    ...
    private async purgeCoupleData(manager: EntityManager, coupleIds: string[]) {
        const uniqueCoupleIds = Array.from(new Set(coupleIds)).filter(Boolean);
        if (uniqueCoupleIds.length === 0) return 0;

        await manager.getRepository(Message).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Event).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Moment).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Location).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Media).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Trip).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(LocationHistory).delete({ coupleId: In(uniqueCoupleIds) });
        await manager.getRepository(Couple).delete({ id: In(uniqueCoupleIds) });

        return uniqueCoupleIds.length;
    }

    private getDisconnectedRetentionCutoff() {
        return new Date(Date.now() - this.disconnectedRetentionMs);
    }
