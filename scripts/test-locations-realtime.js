...
async function main() {
    ...
    await socketReady;
    console.log("Socket connected. Emitting locations:join...");

    socket.on("locations:joined", (payload) => {
        console.log("Received locations:joined event", payload?.coupleId || "");
    });
    ...
    await new Promise((resolve, reject) => {
        ...
        socket.emit("locations:join", {}, (ack) => {
            clearTimeout(timer);
    ...
    console.log("Creating location via REST...");
    ...
    if (createdEvent?.id !== created.id) {
        throw new Error("locations:created payload mismatch");
    }
    console.log("Received locations:created", createdEvent?.id);

    console.log("Updating location via REST...");
    const updatedEventPromise = waitForEvent(
        socket,
        "locations:updated",
        (payload) => payload && payload.id === created.id
    );
    const updated = await apiRequest(`/locations/${created.id}`, {
        method: "PATCH",
        body: JSON.stringify({
            name: "Realtime Test Location Updated",
            radius: 300
        })
    });
    const updatedEvent = await updatedEventPromise;
    if (updatedEvent?.id !== updated.id) {
        throw new Error("locations:updated payload mismatch");
    }
    console.log("Received locations:updated", updatedEvent?.id);

    console.log("Deleting location via REST...");
    const deletedEventPromise = waitForEvent(
        socket,
        "locations:deleted",
        (payload) => payload && payload.id === created.id
    );
    const deleted = await apiRequest(`/locations/${created.id}`, {
        method: "DELETE"
    });
    const deletedEvent = await deletedEventPromise;
    if (deletedEvent?.id !== deleted.id) {
        throw new Error("locations:deleted payload mismatch");
    }
    console.log("Received locations:deleted", deletedEvent?.id);
